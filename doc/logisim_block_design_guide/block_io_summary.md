...
| Branch Comp | `Data_rs`, `Data_rt`, `BrSel[0]` | `BranchTaken` 조건 | `BrSel` | `BrSel=0`은 EQ, `BrSel=1`은 NE입니다. `Branch` enable은 PCControl에서 gating합니다. |
...
| Jump Branch / PCControl | `Branch`, `Jump`, `BranchComp` 조건, `SelectedJumpTarget`, `BranchTarget` | `PCSel` | `Branch`, `Jump`, `PCSel` | 우선순위는 `Jump` > taken branch > `PC+4`입니다. |
